...
function errorMyNetdataMenu() {
    setMyNetdataMenu(`<div class="agent-item" style="white-space: nowrap">
        ...
        无法从netdata.cloud加载已知的netdata代理！
        <div></div>
    </div>`);
}
...
function restrictMyNetdataMenu() {
    setMyNetdataMenu(`<div class="info-item" style="white-space: nowrap">
        <span>请<a href="#" onclick="signInDidClick(event); return false">登录netdata.cloud</a>以查看您的节点！</span>
        <div></div>
    </div>`);
}
...
function renderMyNetdataMenu(machinesArray) {
        ...
        if (!NETDATA.registry.isRegistryEnabled()) {
            html += (
                `<div class="info-item" style="white-space: nowrap">
                    <span>请<a href="#" onclick="signInDidClick(event); return false">登录 netdata.cloud</a> 查看您的节点！</span>
                    <div></div>
                </div>
                <hr />`
            );
    ...
    if (!isSignedIn()) {
        html += (
            `<div class="agent-item">
                <i class="fas fa-tv"></i>
                <a onClick="openAuthenticatedUrl('console.html');" target="_blank">节点<sup class="beta"> 测试版</sup></a>
                <div></div>
            </div>
            <div class="agent-item">
            ...
            <div class="agent-item">
                ...
                <a href="https://github.com/netdata/netdata/tree/master/registry#netdata-registry" target="_blank">这是什么？</a>
                <div></div>
            </div>`
        )
    } else {
            ...
            if (typeof data !== 'undefined' && data !== null && typeof data.machine_guid === 'string' && data.machine_guid === guid) {
                ...
                if (!gotoServerStop) {
                    ...
                    if (gotoServerMiddleClick) {
                        ...
                        document.getElementById('gotoServerResponse').innerHTML = '<b>打开到' + NETDATA.registry.machines[guid].name + '的新窗口<br/><a href="' + verifyURL(finalURL) + '">' + escapeUserInputHTML(url) + '</a></b><br/>(如果失败，请检查弹出窗口拦截器)';
                    } else {
                        document.getElementById('gotoServerResponse').innerHTML += '找到了！它在:<br/><small>' + escapeUserInputHTML(url) + '</small>';
                ...
                if (gotoServerValidateRemaining <= 0) {
                    gotoServerMiddleClick = false;
                    document.getElementById('gotoServerResponse').innerHTML = '<b>抱歉！我找不到该服务器的任何可操作的URL</b>';
                }
            }
        });
    }, (id * 50) + penalty);
    ...
    if (!isSignedIn()) {
        ...
        setTimeout(function () {
            if (gotoServerStop === false) {
                document.getElementById('gotoServerResponse').innerHTML = '<b>已添加此计算机的所有已知URL。</b>';
...
function notifyForSwitchRegistry() {
    ...
    if (n !== '' && n.length === 36) {
        ...
        document.getElementById('switchRegistryResponse').innerHTML = "<b>您输入的ID不是GUID。</b>";
    }
}
    ...
    if (deleteRegistryUrl) {
        if (isSignedIn()) {
            deleteCloudAgentURL(deleteRegistryGuid, deleteRegistryUrl)
                .then((count) => {
                    if (!count) {
                        responseEl.innerHTML = "<b>抱歉，netdata.cloud 拒绝了此命令！</b>";
                        return;
                    }
            ...
            NETDATA.registry.delete(deleteRegistryUrl, function (result) {
                if (result !== null) {
                    ...
                    responseEl.innerHTML = "<b>抱歉，注册表服务器拒绝了此命令！</b>";
                }
            });
        }
    }
...
function renderPage(menus, data) {
    ...
    sidebar += '<li class="" style="padding-top:15px;"><a href="https://github.com/netdata/netdata/blob/master/docs/Add-more-charts-to-netdata.md#add-more-charts-to-netdata" target="_blank"><i class="fas fa-plus"></i> 添加更多图表</a></li>';
    sidebar += '<li class=""><a href="https://github.com/netdata/netdata/tree/master/health#Health-monitoring" target="_blank"><i class="fas fa-plus"></i> 添加更多告警</a></li>';
    sidebar += '<li class="" style="margin:20px;color:#666;"><small>每' +
    ((data.update_every === 1) ? '秒' : data.update_every.toString() + '秒') + '，' +
    'Netdata在' + data.hostname.toString() + '上收集<strong>' +
    data.dimensions_count.toLocaleString() + '</strong>个指标，展示在<strong>' +
    data.charts_count.toLocaleString() + '</strong>个图表中' +
    (isMemoryModeDbEngine ? '' : '，') + // oxford comma
    '并使用<strong>' +
    data.alarms_count.toLocaleString() + '</strong>个告警进行监控。';

    if (!isMemoryModeDbEngine) {
        sidebar += '<br />&nbsp;<br />通过<a href="https://docs.netdata.cloud/docs/configuration-guide/#increase-the-metrics-retention-period" target=_blank>配置Netdata的<strong>历史记录</strong></a>或使用<a href="https://docs.netdata.cloud/database/engine/" target=_blank>DB引擎</a>来获取更多历史数据。'
    }
...
function alarmsUpdateModal() {
    var active = '<h3>已触发的警报</h3><table class="table">';
    var all = '<h3>所有运行中的警报</h3><div class="panel-group" id="alarms_all_accordion" role="tablist" aria-multiselectable="true">';
    var footer = '<hr/><a href="https://github.com/netdata/netdata/tree/master/web/api/badges#netdata-badges" target="_blank">netdata徽章</a>会自动刷新。它们的颜色表示警报的状态：<span style="color: #e05d44"><b>&nbsp;红色&nbsp;</b></span>表示严重，<span style="color:#fe7d37"><b>&nbsp;橙色&nbsp;</b></span>表示警告，<span style="color: #4c1"><b>&nbsp;亮绿色&nbsp;</b></span>表示正常，<span style="color: #9f9f9f"><b>&nbsp;浅灰色&nbsp;</b></span>表示未定义（即没有数据或状态），<span style="color: #000"><b>&nbsp;黑色&nbsp;</b></span>表示未初始化。您可以复制粘贴它们的URL以将其嵌入到任何网页中。<br/>netdata可以发送这些警报的通知。查看<a href="https://github.com/netdata/netdata/blob/master/health/notifications/health_alarm_notify.conf" target="_blank">此配置文件</a>获取更多信息。';
    ...
    NETDATA.alarms.get('all', function (data) {
        ...
        function alarm_lookup_explain(alarm, chart) {
            ...
            if (typeof alarm.lookup_dimensions !== 'undefined') {
                ...
                var x = d.split(',');
                if (x.length > 1) {
                    dimensions = '维度总和 <code>' + alarm.lookup_dimensions + '</code> 的';
                } else {
                    dimensions = '维度 <code>' + alarm.lookup_dimensions + '</code> 的所有值的';
                }
            }
        
            return '<code>' + alarm.lookup_method + '</code> '
                + dimensions + ', 图表 <code>' + alarm.chart + '</code>'
                + ', 开始于 <code>' + NETDATA.seconds4human(alarm.lookup_after + alarm.lookup_before, { space: '&nbsp;' }) + '</code> 到 <code>' + NETDATA.seconds4human(alarm.lookup_before, { space: '&nbsp;' }) + '</code>'
                + ((alarm.lookup_options) ? (', 使用选项 <code>' + alarm.lookup_options.replace(/ /g, ',&nbsp;') + '</code>') : '')
                + '.';
        }
        ...
        if (!count_active) {
            active += '<div style="width:100%; height: 100px; text-align: center;"><span style="font-size: 50px;"><i class="fas fa-thumbs-up"></i></span><br/>一切正常。没有触发警报。</div>';
        } else {
            active += footer;
        }

        if (!count_all) {
            all += "<h4>此系统中没有运行中的警报。</h4>";
        } else {
            all += footer;
        }
        ...
        document.getElementById('alarms_log').innerHTML = '<h3>报警日志</h3><table id="alarms_log_table"></table>';
            ...
            $('#alarms_log_table').bootstrapTable({
                ...
                columns: [
                    ...
                    },
                    {
                        field: 'duration',
                        title: '持续时间',
                        titleTooltip: '报警在此事件之前的上一个状态下持续的时间',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                    ...
                    },
                    {
                        ...
                        title: '触发持续时间',
                        titleTooltip: '报警触发的持续时间，就在此事件之前',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                            return NETDATA.seconds4human(value, { negative_suffix: '', space: ' ', now: 'no time' });
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            if (value === 0) {
                                ...
                                return 'FAILED (返回代码 ' + value.toString() + ')';
                            }
                        },
                        align: 'center',
                        valign: 'middle',
...
function loadSnapshotPreflightEmpty() {
    ...
    loadSnapshotModalLog('success', '浏览快照文件（或将其拖放到此处），然后单击 <b>导入</b> 以渲染它。');
    $('#loadSnapshotImport').addClass('disabled');
};
...
function saveSnapshotModalShowExpectedSize() {
    ...
    saveSnapshotModalLog(priority, '快照每个维度将有' + points.toString() + '个点。预计磁盘上的大小为' + sizemb + ' MB，在浏览器内存中为' + memmb + ' MB。<br/>' + msg);
}
    ...
    loadBootstrapSlider(function () {
        ...
        saveSnapshotSlider = new Slider('#saveSnapshotResolutionSlider', {
            ...
            formatter: function (value) {
                ...
                return value + seconds + '每个数据点' + ((value === options.data.update_every) ? '，服务器默认' : '').toString();
            }
        });
    });
}
