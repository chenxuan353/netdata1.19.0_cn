...
NETDATA.errorCodes = {
    ...
    403: {message: "图表库未启用/加载失败", alert: false},
    404: {message: "未找到图表", alert: false},
    405: {message: "无法从服务器下载图表索引", alert: true},
    406: {message: "从服务器下载的图表索引无效", alert: true},
    407: {message: "无法连接netdata服务器", alert: false},
    408: {message: "Netdata服务器对HELLO的响应无效", alert: false},
    409: {message: "无法访问netdata注册表", alert: false},
    410: {message: "netdata注册表访问失败", alert: false},
    411: {message: "netdata注册表服务器对DELETE的响应无效", alert: false},
    412: {message: "netdata注册表DELETE失败", alert: false},
    413: {message: "netdata注册表服务器对SWITCH的响应无效", alert: false},
    414: {message: "netdata注册表SWITCH失败", alert: false},
    415: {message: "netdata告警下载失败", alert: false},
    416: {message: "netdata告警日志下载失败", alert: false},
    417: {message: "netdata注册表服务器对SEARCH的响应无效", alert: false},
    418: {message: "netdata注册表SEARCH失败", alert: false}
};
        ...
        if (this.hasLegend()) {
            ...
            if (NETDATA.options.current.show_help) {
                $(content).popover({
                    ...
                    content: '您可以单击或点击数值或标签以选择维度。通过按住SHIFT或CONTROL键，您可以启用或禁用多个维度。<br/><small>帮助可以从设置中禁用。</small>'
                });
            }
        } else {
            this.element_legend_childs = {
        ...
        while (len--) {
            ...
            if (!state.library.initialized) {
                if (state.library.enabled) {
                    ...
                    state.error('图表库 "' + state.library_name + '" 未启用。');
                }
            }
